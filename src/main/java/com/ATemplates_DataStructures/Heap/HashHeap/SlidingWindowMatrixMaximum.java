package com.ATemplates_DataStructures.Heap.HashHeap;
...
public class SlidingWindowMatrixMaximum {
    private final static Logger logger = LoggerFactory.getLogger(SlidingWindowMatrixMaximum.class);
    ...
    private void testSlidingWindowMaximum() {
        logger.info("{}", maxSlidingWindow2(new int[][]{
                {1, 5, 3},
                {3, 2, 1},
                {4, 1, 9},
        }, 2));// 13
        ...
        for (int up = 0, down = k - 1; down < n; up++, down++) {
            if (toRight) {
                if (!firstTime) {
                    ...
                    heap.add(sum);
//                    System.out.println("go down sum = " + sum);
                }
                firstTime = false;
                ...
                for (int left = 1, right = k; right < m; left++, right++) {
                    ...
                    heap.add(sum);
//                    System.out.println("go right sum = " + sum);
                }
            } else {
                // go down
                for (int j = m - k; j < m; j++) {
...
//                System.out.println("go down sum = " + sum);
                ...
                for (int left = m - k - 1, right = m - 2; left >= 0; left--, right--) {
                    ...
                    heap.add(sum);
//                    System.out.println("go left sum = " + sum);
                }
            }
