...
public class HugeFileSorting {
    private static final Logger logger = LoggerFactory.getLogger(HugeFileSorting.class);
     ...
     */
    public static List<String> hugeFile2SmallFiles(final String HUGE_FILE_PATH, final String OUTPUT_FOLDER_PATH, final long SIZE_THAT_FITS_MEMORY) {
        ...
        final String hugeFileName = HUGE_FILE_PATH.substring(HUGE_FILE_PATH.lastIndexOf('\\'), HUGE_FILE_PATH.lastIndexOf('.'));
        try (BufferedReader bf = new BufferedReader(new FileReader(new File(HUGE_FILE_PATH)))) {
            ...
            while ((line = bf.readLine()) != null) {
                ...
                for (String word : line.split(" ", -1)) {
                    ...
                    if (hashSet.size() >= SIZE_THAT_FITS_MEMORY) {
                        ...
                        String smallFileName = writeWords2File(sortWords(hashSet), OUTPUT_FOLDER_PATH + hugeFileName + "_chunk_" + fileNumber++ + FILE_SUFFIX);
            ...
            if (hashSet.size() > 0) {
                ...
                String smallFileName = writeWords2File(hashSet, OUTPUT_FOLDER_PATH + hugeFileName + "_chunk_" + fileNumber + FILE_SUFFIX);
                smallFiles.add(smallFileName);
            }
        } catch (IOException e) {
            e.printStackTrace();
     ...
     * @throws IOException
     */
    private static String writeWords2File(Iterable<String> iterableCollection, String FILE_NAME_OF_ONE_CHUNK) throws IOException {
    ...
    private static void printSmallFiles(final List<String> smallFiles, final int topK) {
        logger.info("smallFiles = {}", smallFiles + "\n");
        long totalCount = 0;
...
//                            }
    ...
    private static void createFolderIfNotExists(final String OUTPUT_FOLDER_PATH) {
        ...
        logger.debug("succededMkdirs = {}", succededMkdirs);
    }
