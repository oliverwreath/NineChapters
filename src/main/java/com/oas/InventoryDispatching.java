...
//    log.debug("");
  ...
  public List<int[]> dispatching(int[] currentInventories, int[] minimumInventories, int[] desiredInventories,
    ...
    if (warehouseInventory < diffToMeetMinimum) {
      ...
      log.debug("1st Scenario - stock to the cloest fairness of the minimum.");
      ...
      log.debug("mapID2Result=" + mapID2Result);
      ...
      log.debug("2nd Scenario - stock pass the minimum; then to the cloest fairness of the desired.");
      Map<Integer, Integer> mapID2Result = new HashMap<>();
      log.debug("mapID2Result=" + mapID2Result);
      ...
      log.debug("3rd Scenario - just stock to the fullest desired.");
      for (int i = 0; i < numOfStores; i++) {
